...
class MenuMethod {
    ...
    //获取功能详情
    async GetFunctionDetail(input) {
        let url = `${config.remoteUrl}/api/services/app/Function/GetFunctionDetail`;
        var rs = await AjaxHelper.get(url, input);
        return rs;
    }
    //添加功能
    async AddFunction(input) {
        let url = `${config.remoteUrl}/api/services/app/Function/AddFunction`;
        var rs = await AjaxHelper.post(url, input);
        return rs;
    }
    //修改功能
    async UpdateFunction(input) {
        let url = `${config.remoteUrl}/api/services/app/Function/UpdateFunction`;
        var rs = await AjaxHelper.put(url, input);
        return rs;
    }
    //删除功能
    async DeleteFunction(input) {
        let url = `${config.remoteUrl}/api/services/app/Function/DeleteFunction`;
        var rs = await AjaxHelper.delete(url, input);
        return rs;
    }   
}
